...
class UTAE(nn.Module):
    def __init__(
        ...
        out_conv=[32, 2],  # previous: [32, 20]
        str_conv_k=4,
        str_conv_s=2,
        str_conv_p=1,
        agg_mode="att_group",
        encoder_norm="group",
        n_head=16,  # TRY: 1, 4, 8, 16, 32 Previous: 16
        d_model=256,
        d_k=4,  # TRY: 4, 8
        encoder=False,
        return_maps=False,
        ...
        **kwargs,
        ...
        if aux_conv is not None:
            if len(aux_conv) > 0:
                self.intermediate_flag = True
                self.intermediate_net = ConvBlock(nkernels=[decoder_widths[-1]] + aux_conv, padding_mode=padding_mode)
    ...
    def forward(self, input, batch_positions=None, return_att=False):
        ...
        if self.intermediate_flag:
            inter_out = self.intermediate_net(out)
        ...
        if self.encoder:
            if self.intermediate_flag:
                return out, maps, inter_out
            return out, maps
        else:
            out = self.out_conv(out)
            if return_att:
                if self.intermediate_flag:
                    return out, att, inter_out
                return out, att
            if self.return_maps:
                if self.intermediate_flag:
                    return out, maps, inter_out
